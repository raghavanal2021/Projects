...
deriv_url = os.getenv("DERIV_URL")
logging.info(f"{utilities_url} found !! ")
class DerivativeLoad():
    '''
    Loads Equity files from NSE Website
    '''

    name = "service_derivativeload"
    ...
    @event_handler("service_loader","derivload")
    ...
    def prepareload(self,startDate,endDate,loadoptions):
        try:
            ...
            for dates in datelist:
                fil = self.get_derivative_file_name(dates)
                ...
                if loadcheckresponse['IndexFutures'] == 'N':
                    year_date = fildate.year
                    year_month = datetime.strftime(fildate,'%b').upper()
                    downloadurl = f"{deriv_url}/{year_date}/{year_month}/{fil}"
                    print(downloadurl)
                    status = self.download.downloadfile(downloadurl,fil,'deriv')
                    if status == 100 :
                        message = {"action":"loadresponse","payload":{"eventtimestamp":datetime.now().isoformat(),"asset":'Derivatives','reporttype':'price',"statuscode":200, "statusdesc":f"Derivatives file {fil} is successful.","date":dates}}
                        self.red.publish("loadstatus",json.dumps(message))
                else:
                    message = {"action":"loadresponse","payload":{"eventtimestamp":datetime.now().isoformat(),"asset":'Derivatives','reporttype':'price',"statuscode":-100, "statusdesc":f"Data Already Exists. Not downloading again. ","date":dates}}
                    self.red.publish("loadstatus",json.dumps(message))
            self.mongoload.deriv_load('deriv')
    ...
    def get_derivative_file_name(self,datestr):
        dateobj = datetime.strptime(datestr,'%Y%m%d')
        fileout = f"fo{dateobj.strftime('%d%b%Y').upper()}bhav.csv.zip"
        print (fileout)
        return fileout
